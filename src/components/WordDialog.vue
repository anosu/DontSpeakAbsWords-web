<template>
  <el-dialog class="dialog" v-model="isShowDialog" lock-scroll="true" title="提交词条" width="450px" align-center center
    close-delay="100">
    <el-form label-position="top" size="large" :model="wordData" ref="singleWordForm">
      <el-form-item label="词条（50字符以内）" prop="word" :rules="[
        {
          required: true,
          pattern: /\S+/,
          message: '内容不能为空',
      ...
      <el-form-item label="释义（可不填，100字符以内）" prop="translation" :rules="[
        {
          pattern: /\S+/,
          message: '内容不能为空',
          trigger: ['change', 'blur'],
    ...
    <template #footer>
      ...
      <el-button size="large" type="primary" @click="submitForm(singleWordForm)">确认</el-button>
    </template>
  </el-dialog>
</template>
